...
const EMPTY_FORM = {
  title: "", type: "hotel", description: "", tags: "",
...
export default function ProductsPage({ token, type }) {
  ...
  const openAdd = () => { setForm({ ...EMPTY_FORM, type, markets: MARKETS.map((m) => ({ market: m.key, currency: m.currency, price: 0, offerLabel: "" })), localized: LOCALES.map((l) => ({ locale: l.key, title: "", description: "" })) }); setEditing(null); setTab("basic"); setError(""); setShowForm(true); };

  const openEdit = (p) => {
    const markets = MARKETS.map((m) => {
      const existing = p.markets?.find((x) => x.market === m.key);
      return existing || { market: m.key, currency: m.currency, price: 0, offerLabel: "" };
    });
    const localized = LOCALES.map((l) => {
      const existing = p.localized?.find((x) => x.locale === l.key);
      return existing || { locale: l.key, title: "", description: "" };
    });
  ...
  const setMarket = (i, field, val) => setForm((f) => {
    ...
    markets[i] = { ...markets[i], [field]: val };
    return { ...f, markets };
  });

  const setLocalized = (i, field, val) => setForm((f) => {
    const localized = [...f.localized];
    localized[i] = { ...localized[i], [field]: val };
    return { ...f, localized };
  });

  const typeIcons = { hotel: "🏨", tour: "🗺️", package: "📦", vehicle: "🚗" };

  return (
    <div>
      <div className="page-header">
        <h2>{typeIcons[type]} {type.charAt(0).toUpperCase() + type.slice(1)}s</h2>
        <button className="btn-primary" onClick={openAdd}>+ Add {type}</button>
      </div>

      {/* Product Cards Grid */}
      {products.length === 0 ? (
        <p className="empty">No {type}s yet. Click "Add {type}" to create one.</p>
      ) : (
            ...
            <div className="product-card" key={p._id}>
              ...
              <div className="product-media">
                {p.media?.[0] ? (
                  ...
                  <div className="product-thumb-placeholder">{typeIcons[p.type]}</div>
                )}
              ...
              <div className="product-actions">
                ...
                <button className="btn-icon" title="Duplicate" onClick={() => handleDuplicate(p._id)}>📋</button>
              ...
              {tab === "media" && (
                <div>
                  <div className="media-add-row">
                    <select className="form-input" style={{ width: 100 }} value={mediaType} onChange={(e) => setMediaType(e.target.value)}>
                  ...
                  <p className="form-hint">Paste direct URLs for images (jpg, png, webp) or videos (mp4, YouTube embed, etc.)</p>
                    ...
                    {form.media.map((m, i) => (
                      <div className="media-item" key={i}>
                        {m.type === "video"
                          ...
                          : <img src={m.url} alt="" className="media-preview" onError={(e) => { e.target.src = ""; e.target.alt = "Invalid URL"; }} />
                        }
              ...
              {tab === "inventory" && (
                <div className="form-grid">
                  <label className="form-label">Stock Quantity
                    <input className="form-input" type="number" value={form.inventory.quantity} onChange={(e) => setForm({ ...form, inventory: { ...form.inventory, quantity: e.target.value } })} />
                  </label>
                  <div />
                  <label className="form-label form-check" style={{ gridColumn: "1/-1" }}>
                    <input type="checkbox" checked={form.inventory.stopSales} onChange={(e) => setForm({ ...form, inventory: { ...form.inventory, stopSales: e.target.checked } })} />
                    <span>🚫 Stop Sales — prevent new bookings for this product</span>
                  </label>
                </div>
              )}
